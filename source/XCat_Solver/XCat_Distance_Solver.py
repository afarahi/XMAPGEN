...
def distance_integral_element(Input_Param,z):
    dl = sqrt( Input_Param.Omega_M*(1.0+z)**3 + Input_Param.Omega_DE*(1.0+z)**(3.0*(1.0+Input_Param.w)) + Input_Param.Omega_R*(1.0+z)**4 - Input_Param.Omega_k*(1.0+z)**2 )
    return 1.0/dl
...
def Proper_Distance_Tabulate(Input_Param,z_max): 
   ...
   if (Input_Param.Omega_k > 0.0):
      for i in range(1,integral_n+1):   
         integral_dl += dz*( distance_integral_element(Input_Param,z[i])+distance_integral_element(Input_Param,z[i-1]) ) / 2.0
